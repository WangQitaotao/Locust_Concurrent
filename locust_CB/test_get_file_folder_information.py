...
class GetFileInformation(TaskSet):
    ...
    data = {"id": "root"}
    re1 = requests.post(f"http://209.126.106.37/rest/api/basic/list", json=data, headers=header)  # 调用查看信息接口，获取目录下的文件或文件夹
...
if __name__ == '__main__':
    ...
    os.system("locust -f test_get_file_folder_information.py --headless -u 5 -r 1 -t 10s --skip-log-setup --only-summary --csv=../report/get_file_folder_information/result --html=../report/get_file_folder_information/report.html")
